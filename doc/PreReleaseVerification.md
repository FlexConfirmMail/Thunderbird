...
8. 「送信」を押す。
   ...
   * [ ] 組織外の宛先として以下がグループ化され列挙されている。
     ...
     * `example.com`
       ...
       * `user2@example.com`
       * `user3@example.com`
9. 組織内の宛先の見出し部分のチェックボックスをオンにする。
   * [ ] 組織内の宛先全てにチェックが付く。
   * [ ] 「送信」がグレイアウトしている。
10. 組織外の宛先の見出し部分のチェックボックスをオンにする。
    * [ ] 組織外の宛先全てにチェックが付く。
    * [ ] 「送信」が有効になる。
11. 組織外の宛先の見出し部分のチェックボックスをオフにする。
    * [ ] 組織外の宛先全てのチェックが外れる。
    * [ ] 「送信」がグレイアウトしている。
12. 組織外の宛先の見出し部分のチェックボックスをオンにする。
    * [ ] 組織外の宛先全てにチェックが付く。
    * [ ] 「送信」がグレイアウトしている。
13. 組織内の宛先の見出し部分のチェックボックスをオフにする。
    * [ ] 組織内の宛先全てのチェックが外れる。
    * [ ] 「送信」が有効になる。
14. 「キャンセル」を押して確認のダイアログを閉じる。
15. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
16. 「組織内の宛先の一括チェックを許可」のチェックを外す。
17. 「外部の宛先の一括チェックを許可」のチェックを外す。
18. メール編集ウィンドウに戻る。
19. 「送信」を押す。
    * [ ] チェックボックスがある確認画面が開かれる。
    * [ ] 「送信」がグレイアウトしている。
...
23. 「組織内の宛先の一括チェックを許可」のチェックを入れる。

#### 送信前の宛先確認を行うかどうかの判断

1. 常に送信確認が行われる動作を検証する。
   1. FlexConfirmMailの設定画面を開く。
   2. 「送信前の宛先確認」→「常に確認」を選択する。
   3. 3ペインのタブに切り替える。
   4. 新規にメール作成を開始する。
...
3. 誤送信の危険性がある場合だけ確認する動作を検証する。
   1. 確認が行われない場合を検証する。
      1. FlexConfirmMailの設定画面を開く。
      2. 「送信前の宛先確認」→「誤送信の危険性がある場合だけ確認」を選択する。
      3. 3ペインのタブに切り替える。
   ...
   4. 外部からテキストをコピー＆ペーストした場合を検証する。
      ...
      2. Thunderbird以外のアプリケーションから任意のテキストを10文字程度コピーし、本文に貼り付ける。
         * 「社外秘」という文字列が含まれていると別の送信前確認に該当してしまうため、「社外秘」という文字列は含めないようにする。
      3. 「送信」を押す。
         * [ ] 送信前確認のダイアログが開かれない。
         * [ ] カウントダウンが表示される。
...
4. すべての宛先が社内である場合に確認を省略する動作を検証する。
   1. FlexConfirmMailの設定画面を開く。
   2. 「すべての宛先が組織内のメール送信時は確認ダイアログを表示しない」にチェックを入れる。
   3. 3ペインのタブに切り替える。
   4. 新規にメール作成を開始する。
   ...
   9. 「すべての宛先が組織内のメール送信時は確認ダイアログを表示しない」のチェックを外す。
   10. メール編集ウィンドウに戻る。
   11. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
...
5. 宛先の件数によって確認を省略する動作を検証する。
   1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   2. 「宛先がN件以下の場合は確認ダイアログを表示しない」に「2」を入力する。
   3. メール編集ウィンドウに戻る。
   4. 「送信」を押す。
      * [ ] チェックボックスがある確認画面が開かれない。
      * [ ] カウントダウンが表示される。
   ...
   11. 「宛先がN件以下の場合は確認ダイアログを表示しない」に「0」を入力する。
...
1. ToまたはCcに含まれる外部の宛先のドメインの数によって警告を行う動作を検証する。
   1. 警告が行われない場合を検証する。
      1. FlexConfirmMailの設定画面を開く。
      2. 「ToまたはCcの宛先にN個以上のドメインが含まれる場合に警告する」にチェックを入れる。
      3. 3ペインのタブに切り替える。
      4. 新規にメール作成を開始する。
...
2. 既存メールへの返信で、既存の宛先にないドメインの宛先が追加されたときに強調表示と警告を行う動作を検証する。
   1. 強調表示と警告が行われる場合を検証する。
      ...
      7. 「キャンセル」を押して警告のダイアログを閉じる。
      8. 「キャンセル」を押して確認のダイアログを閉じる。
   2. 強調表示のみ行われる場合を検証する。
      1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
      2. 「返信の宛先に今まで含まれていなかったドメインのアドレスが追加された場合に警告する」のチェックを外す。
      3. メール編集ウィンドウに戻る。
      4. 「送信」を押す。
         * [ ] チェックボックスがある確認画面が開かれる。
         * [ ] `user1@example.com` が赤字で強調表示される。
   ...
   3. 警告のみ行われる場合を検証する。
      1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
      2. 「返信の宛先に今まで含まれていなかったドメインのアドレスが追加された場合に警告する」のチェックを入れる。
   ...
   6. 「返信の宛先に今まで含まれていなかったドメインのアドレスが追加された場合に警告する」のチェックを入れる。
...
2. 添付ファイル名の手動入力を求める場合を検証する。
   1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   2. 「外部ドメイン・アドレスの宛先がある場合、添付ファイル名の手動での入力を求める」にチェックを入れる。
   3. メール編集ウィンドウに戻る。
   4. 「送信」を押す。
      * [ ] チェックボックスがある確認画面が開かれる。
      * [ ] 添付ファイルセクションの見出しにチェックボックスがない。
...
2. 一括チェックの動作を検証する。
   1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   2. 「添付ファイルのチェックボックスの一括制御を許可」にチェックを入れる。
   3. メール編集ウィンドウに戻る。
   4. 「送信」を押す。
      * [ ] チェックボックスがある確認画面が開かれる。
   ...
   8. 「外部ドメイン・アドレスの宛先がある場合、添付ファイル名の手動での入力を求める」のチェックを外す。
   9. メール編集ウィンドウに戻る。
   10. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
...
4. 「添付ファイルのチェックボックスの一括制御を許可」にのチェックを外す。
...
2. メール編集ウィンドウを閉じる。
3. FlexConfirmMailの設定画面を開く。
4. 「その他」→「[送信]ボタンを押してからN秒間のカウントダウン語に実際に送信する」の秒数に「10」を入力する。
...
1. 件名、本文の確認の動作を検証する。
   ...
   9. 「送信」を押す
      ...
      * [ ] 組織外の宛先のグループがすべて同じ色で表示される。
   10. すべてのチェックをオンにする。
       * [ ] すべてにチェックが入った時点で「送信」が有効になる。
   11. 確認のダイアログを小さくリサイズする。
   12. 「キャンセル」を押して確認のダイアログを閉じる。
   13. メール編集ウィンドウを閉じる。
2. 強調表示の切り替えを検証する。
   1. FlexConfirmMailの設定画面を開く。
   ...
   6. 3ペインのタブに切り替える。
   7. 「社外の人とのやり取りのメール」を選択し、「全員に返信」ボタンを押す。
   8. 宛先に `mail@example.com` を追加する。
   9. 「送信」を押す
      * [ ] チェックボックスがある確認画面が開かれる。
      * [ ] ダイアログが前回閉じたときの大きさで開かれている。
   ...
   10. すべてのチェックをオンにする。
       * [ ] すべてにチェックが入った時点で「送信」が有効になる。
   11. 「キャンセル」を押して確認のダイアログを閉じる。
   12. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   13. 「その他」→「確認のダイアログを常に大きなサイズで開く」のチェックを入れる。
   14. 「その他」→「確認のダイアログの冒頭メッセージ」→「メッセージを強調表示する」のチェックを外す。
   15. メール編集ウィンドウに戻る。
   16. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
       * [ ] ダイアログ最上部に「送信前にご確認ください！！」と通所の文字で表示される。
       * [ ] ダイアログウィンドウが大きなサイズで表示される。
   17. 確認のダイアログを小さくリサイズする。
   18. 「キャンセル」を押して確認のダイアログを閉じる。
   19. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
       * [ ] ダイアログウィンドウがリサイズ後のサイズではなく大きなサイズで表示される。
   20. 「キャンセル」を押して確認のダイアログを閉じる。
   21. メール編集ウィンドウを閉じる。
...
1. 既存メールへの返信で、既存の宛先にないドメインの宛先が追加されたときの警告を検証する。
   ...
   6. 「Send」を押す
      * [ ] 追加された宛先の `user1@example.com` の確認を求めるダイアログが表示される。
   7. 「Cancel」を押して警告のダイアログを閉じる。
   8. 「Cancel」を押して確認のダイアログを閉じる。
   9. メール編集ウィンドウを閉じる。
...
1. 既存メールへの返信で、既存の宛先にないドメインの宛先が追加されたときの警告を検証する。
   ...
   6. 「发送」を押す
      * [ ] 追加された宛先の `user1@example.com` の確認を求めるダイアログが表示される。
   7. 「取消」を押して警告のダイアログを閉じる。
   8. 「取消」を押して確認のダイアログを閉じる。
   9. メール編集ウィンドウを閉じる。
