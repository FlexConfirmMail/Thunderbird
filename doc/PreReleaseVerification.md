...
2. 添付ファイル名の手動入力を求める場合を検証する。
   ...
   3. メール編集ウィンドウに戻る。
   4. ~~「送信」を押す。~~（4.2.8で修正済みの不具合のため省略）
      * ~~[ ] チェックボックスがある確認画面が開かれる。~~
      * ~~[ ] 添付ファイルセクションの見出しにチェックボックスがない。~~
      * ~~[ ] 各添付ファイルにチェックボックスがあり、チェック状態を切り替え可能である。~~
   5. ~~すべてのチェックをオンにする。~~（4.2.8で修正済みの不具合のため省略）
      * ~~[ ] すべてにチェックが入った時点で「送信」が有効になる。~~
   6. ~~「キャンセル」を押して確認のダイアログを閉じる。~~（4.2.8で修正済みの不具合のため省略）
...
1. 件名、本文の確認の動作を検証する。
   ...
   10. すべてのチェックをオンにする。
       * [ ] すべてにチェックが入った時点で「送信」が有効になる。
   11. ~~確認のダイアログを1000×500など極端な横長または縦長にリサイズする。~~（4.2.7で修正済みの不具合のため省略）
   12. ~~「送信」を押す。~~（4.2.7で修正済みの不具合のため省略）
       * ~~[ ] カウントダウンが表示される。~~
   13. ~~「キャンセル」を押してカウントダウンのダイアログを閉じる。~~（4.2.7で修正済みの不具合のため省略）
   14. ~~「送信」を押す。~~（4.2.7で修正済みの不具合のため省略）
       * ~~[ ] チェックボックスがある確認画面が開かれる。~~
...
2. 強調表示の切り替えを検証する。
   ...
   15. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
       * [ ] ダイアログ最上部に「送信前にご確認ください！！」と通常の文字で表示される。
