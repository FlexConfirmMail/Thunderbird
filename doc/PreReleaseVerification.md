...
2. カウントダウンの無効化を検証する。
   1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   2. 「その他」→「[送信]ボタンを押してからN秒間のカウントダウン後に実際に送信する」のチェックを外す。
   3. メール編集ウィンドウに戻る。
   4. 「送信」を押す。
      * [ ] 確認のダイアログが開かれない。
      * [ ] カウントダウンが表示されない。
   5. メール送信を中止できればする。
   6. メール編集ウィンドウが残留していれば閉じる。
3. FlexConfirmMailの設定画面を開く。
4. 「その他」→「[送信]ボタンを押してからN秒間のカウントダウン後に実際に送信する」のチェックを入れ、秒数に「10」を入力する。
...
2. 強調表示の切り替えを検証する。
   ...
   16. すべてのチェックをオンにする。
      * [ ] すべてにチェックが入った時点で「送信」が有効になる。
   17. 確認のダイアログを1000×500など極端な横長または縦長にリサイズする。
   18. 「送信」を押す。
       * [ ] カウントダウンが表示される。
   19. 「キャンセル」を押してカウントダウンのダイアログを閉じる。
   20. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
       * [ ] ダイアログウィンドウが前回リサイズ後のサイズではなく大きなサイズで表示される。
   21. 確認のダイアログを1000×500など極端な横長または縦長にリサイズする。
   22. 「送信」を押す。
       * [ ] カウントダウンが表示される。
   23. 「キャンセル」を押してカウントダウンのダイアログを閉じる。
   24. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
       * [ ] ダイアログウィンドウが前回リサイズ後のサイズではなく大きなサイズで表示される。
   25. 「キャンセル」を押して確認のダイアログを閉じる。
