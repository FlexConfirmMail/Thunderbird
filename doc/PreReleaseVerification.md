...
4. 外部の宛先がある場合のみの、本文に対する強調表示と警告を検証する。
   ...
   6. 本文を以下の通り入力する。
      ...
      5行目
      ```
   7. すべての添付ファイル、すべての宛先を削除する。
   8. 宛先に `piro-test+1@clear-code.com` を追加する。
   9. 「送信」を押す。
      * [ ] チェックボックスがある確認画面が開かれる。
   ...
   11. 「送信」を押す
       ...
       * [ ] カウントダウンが表示される。
   12. 「キャンセル」を押してカウントダウンのダイアログを閉じる。
   13. 宛先に `piro.outsider.reflex+1@gmail.com` を追加する。
   14. 「送信」を押す。
       * [ ] チェックボックスがある確認画面が開かれる。
   ...
   19. 「送信」を押す。
       * [ ] カウントダウンが表示される。
   20. 「キャンセル」を押してカウントダウンのダイアログを閉じる。
5. 送信を禁止する宛先を検証する。
