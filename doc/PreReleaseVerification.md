...
5. 社内と社外の宛先が混在する場合に社内の宛先だけ確認を省略する動作を検証する。
   1. FlexConfirmMailの設定画面を開く。
   2. 「宛先」→「組織外の宛先を含む場合も、組織内の宛先のチェックを省略する」にチェックを入れる。
   3. 3ペインのタブに切り替える。
   4. 新規にメール作成を開始する。
   5. 宛先に `piro-test+1@clear-code.com, external@example.com` を追加する。
   6. 「送信」を押す。
      * [ ] チェックボックスがある確認画面が開かれる。
   ...
   9. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
   10. 「宛先」→「組織外の宛先を含む場合も、組織内の宛先のチェックを省略する」のチェックを外す。
6. 宛先の件数によって確認を省略する動作を検証する。
   1. メール編集ウィンドウをそのまま残し、FlexConfirmMailの設定画面を開く。
