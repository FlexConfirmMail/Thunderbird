...
prepare_msi_sources() {
  ...
  echo -e "copy $host_name.windows.json $host_name.json /S /I \r" >> "$build_msi_bat"
  echo -e "xcopy \"%SOURCE%*\" \"%MSITEMP%\" \r" >> "$build_msi_bat"
  ...
  echo -e "del host.exe $host_name.json \r" >> "$build_msi_bat"
